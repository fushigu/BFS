...
#define DUMPERROR(text) { printf("Validation Error: %s, edge %llu %llu weight %f pred0 %llu pred1 %llu dist0 %f dist1 %f\n",text,(long long unsigned int)v0,(long long unsigned int)v1,w,(long long unsigned int)predv0,(long long unsigned int)predv1,distv0,distv1); val_errors++; return; }
...
vweights=weights;
	...
	for (i = 0; i < nlocalverts; ++i)
		...
		printf("Validation Error: predecessor %llu of vertex %llu is out of range\n",(long long unsigned int)pred[i],(long long unsigned int)VERTEX_TO_GLOBAL(my_pe(),i)),val_errors++;
	...
	printf("Validation Error: wrong nedge_traversed %llu (correct number is %llu)\n",(long long unsigned int)*nedges_in,(long long unsigned int)nedges_traversed),val_errors++;
